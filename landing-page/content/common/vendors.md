...
IOMETE is a fully-managed (ready to use, batteries included) data platform. IOMETE optimizes clustering, compaction, and access control to Iceberg tables. Customer data remains stored in the customer account to prevent vendor lock-in. The core of the IOMETE platform is a serverless lakehouse that leverages Apache Iceberg as it's **core table format**. The IOMETE platform also includes Serverless Spark Jobs, an SQL Editor, a Data Catalog, and Central Data Access Control.  IOMETE supports cross-cloud, on-prem, private cloud, and hybrid cloud setups.
